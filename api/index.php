...
class App
{
    ...
    public function setInfo()
    {
        ...
        $by_x = "60";
        $by_y = "45";
        $by_font = $this->path . "font/minecraft.ttf"; //字体
        $by_color = imagecolorallocate($this->drawIm, 0, 51, 78); //颜色RGB
        //自定义文字
        $diy_size = "20"; 
        $diy_angle = "0"; 
        $diy_x = "125";
        $diy_y = "45";
        $diy_font = $this->path . "font/minecraft.ttf"; 
        $diy_color = imagecolorallocate($this->drawIm, 0, 51, 78); 
        //日期
        $time_size = "10"; 
        $time_angle = "0"; 
        $time_x = "167";
        $time_y = "194";
        $time_font = $this->path . "font/minecraft.ttf"; 
        $time_color = imagecolorallocate($this->drawIm, 0, 51, 78); 
        //位置
        $local_size = "10"; 
        $local_angle = "0"; 
        $local_x = "165";
        $local_y = "212";
        $local_font = $this->path . "font/minecraft.ttf"; 
        $local_color = imagecolorallocate($this->drawIm, 0, 51, 78); 
        //IP
        $ip_size = "10"; 
        $ip_angle = "0"; 
        $ip_x = "165";
        $ip_y = "232";
        $ip_font = $this->path . "font/minecraft.ttf"; 
        $ip_color = imagecolorallocate($this->drawIm, 0, 51, 78); 
        //系统
        $system_size = "10"; 
        $system_angle = "0"; 
        $system_x = "278";
        $system_y = "212";
        $system_font = $this->path . "font/minecraft.ttf"; 
        $system_color = imagecolorallocate($this->drawIm, 0, 51, 78); 
        //浏览器
        $bro_size = "10"; 
        $bro_angle = "0"; 
        $bro_x = "278";
        $bro_y = "232";
        $bro_font = $this->path . "font/minecraft.ttf"; 
        $bro_color = imagecolorallocate($this->drawIm, 0, 51, 78); 
    ...
    public function setWeather()
    {
        ...
        imagettftext($this->drawIm, 14, 0, 95, 140, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/minecraft.ttf", $this->weather['weather']); //今天天气添加到图片
        imagettftext($this->drawIm, 13, 0, 160, 90, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/minecraft.ttf", '温度:' .  $this->weather['temp'] . '℃'); //当前温度添加到图片
        imagettftext($this->drawIm, 13, 0, 237, 90, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/minecraft.ttf", '湿度:' .  $this->weather['humidity'] . '%RH'); //当前湿度添加到图片
        imagettftext($this->drawIm, 11, 0, 160, 115, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path .  "font/minecraft.ttf", '风向:' .  $this->weather['winddirection'] . '方'); //当前风向添加到图片
        imagettftext($this->drawIm, 11, 0, 237, 115, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/minecraft.ttf", '风力级别:' . $this->weather['windpower'] . '级'); //风力级别添加到图片
        imagettftext($this->drawIm, 11, 0, 160, 135, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/minecraft.ttf", '更新时间:' . $this->weather['reporttime']); //更新时间添加到图片
